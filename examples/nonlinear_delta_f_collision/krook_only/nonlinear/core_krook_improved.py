...
@njit(parallel=True)
def weight_push_exact(x, v, w, p, E_c, E_s, t, ds, ub, vb, beta, nonlinear=False):
    """Weight sub-operator (delta-f analogue of F^(2)), frozen x, E_c, E_s.
    ...
    that kick into the delta-f weight equation. Linear mode uses the
    partially linearized p=1 limit. Nonlinear mode evolves Hu-Krommes'
    second weight p=f0/F independently, which is essential once a Krook
    source makes dw/dt nonzero even without field drive.
    """
    N = len(x)
    ...
    for i in prange(N):
        ...
        if nonlinear:
            field_drive = p[i] * dw
            w[i] = w[i] + field_drive - beta * w[i] * ds
            p[i] = p[i] - field_drive
        else:
...
def run_timestepping(sim_params):
    ...
    for n in range(n_steps):
        ...
        if splitting_method == "strang":
            ...
            if not full_f:
                w, p = weight_push_exact(
                    x, v, w, p, E_c, E_s, t, dt / 2.0, ub, vb, beta, nonlinear
                )
            if nonlinear:
            ...
            if not full_f:
                w, p = weight_push_exact(
                    x, v, w, p, E_c, E_s, t + dt / 2.0, dt / 2.0, ub, vb, beta, nonlinear
                )
            if nonlinear:
            ...
            if not full_f:
                w, p = weight_push_exact(x, v, w, p, E_c, E_s, t, dt, ub, vb, beta, nonlinear)
            if nonlinear:
